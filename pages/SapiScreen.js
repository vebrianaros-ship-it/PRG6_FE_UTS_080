...
import { getHewanByCategory, deleteHewan } from '../services/api';
...
export default function SapiScreen({ navigation }) {
  ...
  const fetchData = async () => {
    const response = await getHewansByCategory('sapi');
  ...
  return (
    <View style={styles.container}>
      <FlatList
        ...
        ListEmptyComponent={
          <Text style={styles.emptyText}>Tidak ada data sapi</Text>
        }
      />
    </View>
  );
...
const styles = StyleSheet.create({
  ...
  name: { fontSize: 16, fontWeight: 'bold', color: '#51A9FF' },
